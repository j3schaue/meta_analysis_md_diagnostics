...
dd_mi <- mice(dd_r, # impute missing Xs 
              m = m, 
              method = "midastouch", print = FALSE, predictorMatrix = pred) # Completely random Xs
...
emp_bias_parmas <- com %>% 
  group_by(.imp) %>%
  summarize(H0_g1 = sum(g1hi == 0 & g1hi_na==TRUE)/sum(g1hi == 0), # H0 for X1
...
######## JL updated version
write.csv(ebp, file = "../data/cca_bias_table_JL.csv")
```

...
dd_mi <- mice(dd_r, m = m) # We can revise to completely random imputation.
...
emp_bias_parmas <- com %>% 
  group_by(.imp) %>%
  summarize(H0_g1 = sum(g1hi == 0 & g1hi_na)/sum(g1hi_na == 0), 
            H1_g1 = sum(g1hi == 1 & g1hi_na)/sum(g1hi_na == 1), 
            H0_g2 = sum(g1hi == 0 & g1hi_na)/sum(g1hi_na == 0), 
            H1_g2 = sum(g1hi == 1 & g1hi_na)/sum(g1hi_na == 1), 
            p01 = sum(g2hi == 1 & g1hi == 0)/sum(g1hi == 0),
            p11 = sum(g2hi == 1 & g1hi == 1)/sum(g1hi == 1),
            v0_g1 = sum(v_g * (g1hi == 0))/sum(g1hi == 0), 
            v1_g1 = sum(v_g * (g1hi == 1))/sum(g1hi == 1), 
            v0_g2 = sum(v_g * (g2hi == 0))/sum(g2hi == 0), 
            v1_g2 = sum(v_g * (g2hi == 1))/sum(g2hi == 1),
            psi1_g1 = glm(I(1-g1hi_na) ~ g1hi + es_g, family = "binomial")$coefficients[["es_g"]],
            psi1_g2 = glm(I(1-g2hi_na) ~ g2hi + es_g, family = "binomial")$coefficients[["es_g"]])

betas <- lapply(1:m, 
                FUN = function(i){
                  dat <- com %>% filter(.imp == i)
                  fit <- robu(es_g ~ g1hi + g2hi, data = dat, studynum = studyid, var.eff.size = v_g)
                  mod <- fit$reg_table
                  beta1 <- mod[[2, 2]]
                  beta2 <- mod[[3, 2]]
                  tau2 = as.numeric(sensitivity(fit)$`Rho = 0.2`[7])

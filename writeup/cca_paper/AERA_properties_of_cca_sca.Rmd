...
author: 
- Jacob Schauer^1^, Jihyun Lee^2^, Karina G. Diaz^3^, Therese D. Pigott^4^
- ^1^Northwestern University, ^2^The University of Texas at Austin, ^3^University of Pennsylvania, 
...
Analyses of these models in the absence of missing data have been studied thoroughly in the literature [@berkeyRandomeffectsRegressionModel1995; @hedgesCombiningIndependentEstimators1983; @hedgesRobustVarianceEstimation2010; @konstantopoulosFixedEffectsVariance2011;
...
Ignoring missing data can potentially lead to biased estimates of parameters of interest [@littleStatisticalAnalysisMissing2002; @grahamMissingData2012].
...
Tanner-Smith et al. [@tanner-smithAdolescentSubstanceUse2016] conducted a meta-analysis that examined the effects of substance abuse interventions on future substance use among adolescents. 
...
Table \@ref(tab:misspat) shows that only 37 of the `r n_effect` (50%) have a reported treatment intensity for both groups (i.e., $X_{1}$ and $X_{2}$ are both observed), but that 54 (73%) of effects report Group 1's treatment intensity (i.e., $X_{1}$ is observed) and 41 (55%) effects report Group 2's treatment intensity.
```{r, echo = F}
knitr::kable(mitab, 
             digits = 2,
             format = "latex", 
...
knitr::kable(tabcc, #rv_hiint_tab, 
             ...
             escape = FALSE,
             align = 'lccccc', 
             caption = "\\label{tab:ccadtexample} \\textit{This table displays the meta-regression results for the model regressing effect sizes on high-intensity indicator variables when using complete- and shifting-case analyses.}")
```
...
This assumption is true of some effect size indices and is a very accurate large-sample approximation for others [@cooperHandbookResearchSynthesis2019].
...
It is worth pointing out that the *complete-data model*, which refers to the model with no missing data, is distinct from the *complete-case analysis*, which is an estimation procedure that conditions only on observed data.
...
A common concern in meta-regression is that models may not be able to account for all relevant covariates, either due to sample size constraints or because some covariates were not observed [@lipseyThoseConfoundedModerators2003].
...
It has been noted that there are various conditions under which components of $\beta_S$ are unequal to their counterparts in $\beta$ [@farrarMulticollinearityRegressionAnalysis1967].
For instance, it can be the case that $\beta_S = [\tilde{\beta}_0, \tilde{\beta}_1] \neq [\beta_0, \beta_1] \subset \beta$.
The difference between $\beta_S$ and components of $\beta$ is often referred to as *omitted variable bias* in the statistical and econometric literature [@melaImpactCollinearityRegression2002; @angristpischke2009]. 
...
Rubin [@rubinInferenceMissingData1976] defined three types of mechanisms in terms of the distribution of $R$.
...
In general, it is impossible to know whether a selection model is correctly specified, but the formulation in \@ref(eq:r-loglinear) offers a few important advantages. 
First, it is fairly general: the only assumption made of the basis functions $f_{mj}$ is that they are differentiable, which means model \@ref(eq:r-loglinear) allows for nonlinear or interaction terms.
...
If $p(T_i | X_i, v_i)$ is the standard fixed- or random effects meta-regression model in equation \@ref(eq:full-data-prob), and $p(R_i \in \mathcal{R}_j | T_i, X_i, v_i)$ follows the log-linear model in \@ref(eq:r-loglinear), and the $f_{mj}$ are differentiable with respect to $T_i$, then
\begin{equation}
...
Given the selection model in \@ref(eq:cc-loglinear), the bias of the complete-case estimator for the intercept, $\hat{\beta}_{0C}$, is:
\begin{equation}
...
There are various conventions for the size of an odds ratio that depend on base rates $P[R = \mathbbm{1} | T]$ that could be interpreted as ranging from 1.5 to as large as 9.0, though various researchers have noted that odds ratios greater than 3.0 or 4.0 could be considered large [@cohenStatisticalPowerAnalysis1988; @fergusonEffectSizePrimer2009; @chenHowBigBig2010; @haddockUsingOddsRatios1998].
...
Based on conventions in the social and medical sciences (some arbitrary, some empirical), meaningful values of $D_T$ might feasibly range from 0.2 to 1.0 [@cohenStatisticalPowerAnalysis1988; @hillBenchmarks2008].
...
Thus, $|p(R = 0 | X = 1) - p(R = 0 | X = 0)|$ could be as small as 0, but could possibly be as large as 1; arbitrary conventions on the size of correlations suggest that $|p(R = 0 | X = 1) - p(R = 0 | X = 0)| = 0.5$ would be a "large" value [@cohenStatisticalPowerAnalysis1988].
...
Recall that the mechanism in these computations is assumed to be MNAR, since $\psi_2$ in \@ref(eq:cc-loglinear) is nonzero. 
...
<!-- Suppose we assume the equal distribution for $p(X=0|R=0)$ and $p(X=1|R=0)$, so that 10 among 20 missing values would be $X_1=0$. Then, we can approximate $H_1(0)$ to be 21% (i.e., $\frac{10}{48}$). Using a logit model analogous to \@ref(eq:cc-loglinear), we get an estimate of $\psi_1 =$. 
...
Note that the $\delta_{ij}$ are the biases due solely to missingness as in equation \@ref(eq:bias-delta).

The expression in \@ref(eq:bias-sca) shows that a shifting-case analysis suffers from two sources of bias. 
The first source, captured in the first term in \@ref(eq:bias-sca), is a function of the coefficients for the excluded covariates $\beta_E$, which we refer to as as omitted variable bias. 
Discussion in a previous section argued that the term *omitted variable bias* assumes that $\beta$ is the target of inference in an SCA, which may or may not be the case.
...
Note that model \@ref(eq:sca-ex) inherently specifies $\beta_0, \beta_1$, and $\beta_2$ as parameters of interest.
...
Even if we assume that the mechanism follows a log-linear model like that in \@ref(eq:cc-loglinear) or \@ref(eq:logit-sca), the resulting formulas for the bias depend on quantities, such as $\psi$ and $\eta$ that are not known, and cannot be estimated in the presence of missing data without further assumptions.
...
In the same vein as multiple imputation (MI), each set of imputed values constitutes a "complete" dataset from which we can compute the parameters relevant to bias [@rubinMultipleImputationNonresponse1987; @littleStatisticalAnalysisMissing2002]. 
...
Thus, we would urge interpretation of the following results as *potential* biases in the CCA and SCA estimators presented earlier in this article, rather than a precise estimate of the bias.
We generated $m = 1,000$ imputations using the `mice` software in the `R` programming language [@vanbuurenMiceMultivariateImputation2011].
Estimates of $\eta$ were computed using `metafor`, specifying a Paule-Mandel estimator for the variance component $\tau^2$ [@viechtbauerConductingMetaanalysesMetafor2010]. 
To estimate the log-linear model selection parameters $\psi$ in \@ref(eq:logit-sca), as well as $H(X)$, we used a logistic regression with the missingness indicator $R_{ij}$ and $T_i$ and $X_{ij}$ as the predictors.
...
As noted above, the resulting estimators of the intercept $\beta_0$ and slope $\beta_1$ will exhibit bias due to missingness given in \@ref(eq:cc-bias-b0) and \@ref(eq:cc-bias-beta1-example) and bias due to omitting variables as in \@ref(eq:omvar-b0) and \@ref(eq:omvar-b1).
...
Omitted variable bias for $\hat{\beta}_{0S}$ ranges from `r round(summary(ebp$omv_bias_b0)["Min."], 2)` to `r round(summary(ebp$omv_bias_b0)["Max."], 2)` with a mean of `r round(summary(ebp$omv_bias_b0)["Mean"], 2)`; omitted variable bias for $\hat{\beta}_{1S}$ ranges from `r round(summary(ebp$omv_bias_b1)["Min."], 2)` to `r round(summary(ebp$omv_bias_b1)["Max."], 2)` with a mean of `r round(summary(ebp$omv_bias_b1)["Mean"], 2)`.
Similarly the bias due to missingness could feasibly range from `r round(summary(ebp$bias_b0CC)["Min."], 2)` to  `r round(summary(ebp$bias_b0CC)["Max."], 2)` with a mean of `r round(summary(ebp$bias_b0CC)["Mean"], 2)` for $\hat{\beta}_{0S}$, while the missingness bias of $\hat{\beta}_{1S}$ might range from `r round(summary(ebp$bias_b1CC)["Min."], 2)` to `r round(summary(ebp$bias_b1CC)["Max."], 2)` with a mean of `r round(summary(ebp$bias_b1CC)["Mean"], 2)`
...
In sum, this amounts to a total bias of `r round(summary(ebp$bias_b0CC + ebp$omv_bias_b0)["Min."], 2)` to `r round(summary(ebp$bias_b0CC + ebp$omv_bias_b0)["Max."], 2)` for $\hat{\beta}_{0S}$ and from `r round(summary(ebp$bias_b1CC + ebp$omv_bias_b1)["Min."], 2)` to `r round(summary(ebp$bias_b1CC + ebp$omv_bias_b1)["Max."], 2)` for $\hat{\beta}_{1S}$.
...
<!-- Imputations relied on logistic regression approach (`logreg`) for binary covariates in `mice`.  -->
...
<!-- \textcolor{orange}{No preference on imputation model here, Jihyun, since the results are pretty similar. The logreg option may be simpler to explain, but we may get pushback on it.} -->

<!-- \textcolor{orange}{How about this: Boxplots of bias for beta0 and beta1. Two horizontal panels: one for beta0, one for beta1. In each panel, boxplot of (a) total bias, (b) missingness bias, (c) omitted variable bias.  -->
<!-- If we really wanted to, we could add beta2 to this, too.} -->
...
Further, the issues of multicollinearity and confounding in meta-regression, including those discussed by Lipsey @lipseyThoseConfoundedModerators2003, would suggest that omitting variables in an SCA are likely to induce bias.
...
We would also suggest analysts investigate the feasibility of alternative estimation methods. Ibrahim [@ibrahimIncompleteDataGeneralized1990] describes an EM algorithm for generalized linear models with missing covariates, and Ibrahim, lipsitz, and Chen [@ibrahimMissingCovariatesGeneralized1999] extend that algorithm when covariates are MNAR.
...
Finally, multiple imputation has become something of a standard approach for handling missing data across a number of fields [@rubinMultipleImputationNonresponse1987; @littleStatisticalAnalysisMissing2002; @vanbuurenFlexibleImputationMissing2018]. 

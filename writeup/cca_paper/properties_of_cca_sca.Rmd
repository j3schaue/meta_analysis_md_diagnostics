...
\textcolor{blue}{\textit{[Comment-J] When would be the best start using a term "shifting-case"?}}
...
\textcolor{blue}{\textit{[Comment-J] What about the terminology "mixed-effect" meta-regression model? Although not sure whether this makes a difference in this context...}}
...
\]
\textcolor{blue}{\textit{[Comment-J] Maybe "$=p(\mathbf{R|\psi})$" to be consistent with MAR notation below?}}
...
\textcolor{blue}{\textit{[Comment-J] When it says "the sources," does it indicate missingness mechanism?}}
...
\label{eq:pr_xv}
\end{equation}
\textcolor{blue}{\textit{[Comment-J]Can I say that this denominator is to marginalize the distribution of R (across $T_i$)?}}
...
\textcolor{blue}{\textit{[Comment-J] I'd like to clarify my understanding with an example scenario in page 5. In the example, $R_1=[1,1], R_2 = [1,0]$ which are included in the analysis. Then, $\mathcal{R}_j = \{[1,1], [1,0]\}$. Then, $\mathcal{R}=\{[1,1],[1,0],[0,1],[0,0]\}$ which include the cases with unobserved $X_1$. Thus, as the probability of the excluded pattererns ($\{[0,1], [0,0]\}$) gets greater, the bias also becomes greater. Am I understanding the notations correctly?}}
...
\label{eq:cc_mnar}
\end{equation}
\textcolor{blue}{\textit{[Comment-J] Please discard this revision if you intentionally excluded $\eta$ and $\psi$ from the equation.}}
...
The bias of $\hat{\beta}_{1C}$ also increases as a function of $p(R = 0 | X = 1) - p(R = 0 | X = 0)$, which can be thought of as a difference in observation rates \textcolor{blue}{(\textit{[Comment-J] You mean "missingness rates" ($R=0$), am I understanding correctly?})} between $X_i = 1$ and $X_i = 0$.
...
\textcolor{blue}{\textit{[Comment-J] I am getting 1.0 for the largest possible value, if we assume the largest gap between the cells (n-0). Could you explain me on this later during our meeting?}}
...
\label{eq:sca_model}
\end{equation}
\textcolor{blue}{\textit{[Comment-J] Are we omitting the parameters intentionally for the simplicity here ($\eta, \psi$)?}}
...
\textcolor{blue}{\textit{[Comment-J] Is this model essentially assuming MNAR case, because $X_{i1}$ is included in the missingness model? If yes, how about stating the assumption briefly?}}
...
Although FIML for meta-regression model is available in SEM framework [@cheung2015], the approach has not been empirically validated under various conditions. \textcolor{blue}{\textit{[Comment-J] Please discard this addition if it would be better without this.}}

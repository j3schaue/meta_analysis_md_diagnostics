...
This is because the formulas for the bias often depend on quantities that are not known and cannot be estimated from the data.
For example, \@ref(eq:cc-bias-b0) and \@ref(eq:cc-bias-beta1-example) are functions of $H_1(X)$ and $\psi_1$, both of which are functions of or must be estimated with values of the covariates in the meta-regression model that are missing.

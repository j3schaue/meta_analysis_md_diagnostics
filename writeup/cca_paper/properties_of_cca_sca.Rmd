...
We find that bias is highly dependent on the the precise mechanism by which data are missing, and is less reliant on more traditional missingness mechanism classifications (e.g., missing at random vs. not at random).
...
The resulting formulas for the complete-data likelihood function and coefficient estimators will be more complex (including a variance-covariance weight matrix), but they will have a (relatively) similar form as the independent effect size model; indeed, coefficient estimates for the dependent effect size model are also linear transformations of the data.
...
It is worth pointing out that the *complete-data* model, which involves all observed and observed data, is distinct from the *complete-case analysis* which conditions only on observed data.
...
However, when $R_i$ is not independent of $T_i$ (given $X_i$ or $v_i$), then CCA can be biased. 
...
Assume that the selection model is log-linear, and that for the sake of simplicity missingness depends on the size of the effect $T_i$ and the value of $X_i$:
\begin{equation}
\text{logit}[p(R_i = 1 | T_i, X_i, v_i)] 
  = \psi_0  + \psi_1 T_i + \psi_2 X_i
\label{eq:cc-loglinear}
\end{equation}
Note that this is an MNAR mechanism, since the probability of being obesrved depends on $X_i$, however it differs from that in \@ref(eq:cc-mnar) in that it depends on $T_i$. 
A MAR mechanism would involve $\psi_2 = 0$ in equation \@ref(eq:cc-loglinear).
Because \@ref(eq:cc-loglinear) depends on $T_i$, $\delta_{ij} \neq 0$ for this selection model, and hence the CCA estimators may be biased.
...
The mechanism in these computations is assumed to be MNAR, since $\psi_2$ in \@ref(eq:cc-loglinear) is nonzero. 
A MAR mechanism would require $\psi_2 = 0$. 
In that case, the bias for the CCA intercept estimator is identical to the results above.
However, the bias in the slope will be slightly different when $\psi_2 = 0$. 
This is because, as noted noted in \@ref(eq:cc-bias-b1-simp), the bias in the intercept depends (loosely) on the correlation between $R$ and $X$. 
Given the form of $H_1(X)$ in this example, it is possible for the bias of $\hat{\beta}_{1CC}$ to be greater when $\psi_2 \neq 0$ (MNAR) than when $\psi_2 = 0$ (MAR), which can occur if the correlation between $R$ and $T$ and $R$ and $X$ are in the same direction (i.e., $\psi_1, \psi_2$ are in the same direction).
The bias of $\hat{\beta}_{1CC}$  when $\psi_2 \neq 0$ (MNAR) can be smaller in magnitude than when $\psi_2 = 0$ if $\psi_1$ and $\psi_2$ are in opposite directions.

[NOTE1: We should run simulations to check]
[NOTE2: We whould re-write the empirical example stuff, since I don't think it's well-justified]

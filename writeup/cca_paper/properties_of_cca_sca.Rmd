...
Let $T_i$ be the estimate of the effect parameter $\theta_i$, and let $v_i$ be the estimation error variance of $T_i$. Denote a vector of covariates that pertain to $T_i$ as $X_i =[1, X_{i1}, \ldots, X_{i,p-1}]$. Note that the first element of $X_i$ is a 1, which corresponds to an intercept term in a meta-regression model, and that model can be expressed as:
\begin{equation}
...
\end{equation}
where $\mathbf{T} \in \mathbb{R}^k$ is the vector of effect estimates, $\mathbf{v} \in \mathbb{R}^k$ is the vector of estimation variances, and $\mathbf{X} \in \mathbb{R}^{k \times p}$ is the matrix of covariates where each row of $\mathbf{X}$ is simply the row vector $X_i$. 
...
For instance, if two covariates $X_1$ and $X_2$ are of interest, an available-case analysis often involves regressing effects $T_i$ on $X_{i1}$ (excluding $X_{i2}$) and then on $X_{i2}$ (excluding $X_{i1}$). 
Analyses of this sort have been referred to as *shifting units of analysis* in the meta-analytic literature [@cooperResearchSynthesisMetaanalysis2017]. In this article we refer to this analytic approach as a *shifting-case analysis* (SCA).
...
Equation (\ref{eq:bias_gen}) shows that the bias induced by complete- or shifting-case analyses will be a function of the conditional expectation $E[T_i | X_i, v_i, R_i]$, and how it differs from $E[T_i | X_i, v_i] = X_i^T\beta$.
...
Thus, the bias of a complete- or shifting-case analysis will be a function of $\beta$ as well as the missingness mechanism.
...
\begin{equation}
E[T_i | X_i, v_i, R_i = r] \approx X_i^T\beta + (1 - G_j(X_i^T\beta, X_i, v_i))(\tau^2 + v_i)\sum_{m \in \mathcal{D}} \psi_{mj} f_{mj}'(X_i^T \beta, X_i, v_i)
\label{eq:conditional_expectation}
\end{equation}
where $f_{mj}'(X_i^T\beta, X_i, v_i) = \frac{\partial}{\partial T_i} f_{mj}(T_i, X_i, v_i)\rvert_{T_i = X_i^T \beta}$.
Therefore, the bias of the conditional expectation is given by:
\begin{equation}
\text{Bias}[T_i | X_i, v_i, R_i = r] \approx (1 - G_j(X_i^T\beta, X_i, v_i))(\tau^2 + v_i)\sum_{m \in \mathcal{D}} \psi_{mj} f_{mj}'(X_i^T \beta, X_i, v_i)
...
E[T | X, v, R = r_j] 
  ...
  & \approx \int \frac{T \exp\left\{-\frac{1}{2(\tau^2 + v)}\left(T^2 - 2TX\beta - 2T (\tau^2 + v)\sum_{i} \psi_{ij} f_{ij}'(X^T\beta, X, v)\right.\right.}{g_j(X, v)\sqrt{2 \pi (\tau^2 + v)}} \\
  ...
  & = X\beta + (1 - G_j(X, v))(\tau^2 + v)\sum_{i} \psi_{ij} f_{ij}'(X^T \beta, X, v)  \qquad\qquad\qquad \blacksquare
\end{align*}

Note that this uses a first order Taylor expansion of the log-linear model at $T = X^T \beta$, and thus assumes the $f_j$ are differentiable.
...
\[
p(R_i = 1 | T_i, X_i, v_i) \propto \frac{\exp\{\psi_0  + \psi_1 X_i + \psi_2 T_i + \psi_3 T_i X_i\}}{1 + \exp\{\psi_0  + \psi_1 X_i + \psi_2 T_i + \psi_3 T_i X_i\}}
\]
Under this model, $H_{ij} = H_i$ depends only on $X_i$ and not $v_i$, so we can write $H_{ij}$ as:
\[
H_{ij} = H(X_i) = 1 - p(R = \mathbbm{1} | T_i, X_i, v_i)\rvert_{T_i = X_i^T \beta}
\]
Note that $f_{1j}(T_i, X_i, v_i) = X_i$, $f_{2j}(T_i, X_i, v_i) = T_i$, and $f_{3j}(T_i, X_i, v_i) = T_iX_i$. 
Thus, we can write
\begin{align*}
E[T_i | X_i, v_i, R_i = 1] 
  & = \beta_0 + \beta_1 X_i + H(X_i)(v_i + \tau^2)(\psi_2 + \psi_3 X_i)
\end{align*}
...
\begin{align*}
E[\hat{\beta}_0] 
  & = \beta_0 + \frac{H(0)m_0}{\sum_{X_i = 0} w_i}\psi_2 \\
\end{align*}
...
First, it depends on the parameter $\psi_2$ of the selection model. 
When $\psi_2 > 0$ and hence when larger effect estimates are more likely to be missing covariates, the bias is negative.
The bias is positive when $\psi_0 < 0$, which occurs when larger effect estimates are less likely to be missing covariates. 
The other quantity is $H(0)$ which is, roughly, the probability that a given case is missing the covariate when that covariate is $0$. 
In other words, the bias is a function of $\psi_2$ and the proportion of incomplete cases when $X_i = 0$.
...
\begin{align*}
E[\hat{\beta}_1]
  & = \beta_0 + \beta_1 + \frac{H(1) m_1}{\sum_{X_i = 1} w_i}(\psi_2 + \psi_3) - \beta_0 - \frac{H(0) m_0}{\sum_{X_i = 0} w_i}\psi_2 \\
  & = \beta_1 + \frac{H(1) m_1}{\sum_{X_i = 1} w_i}(\psi_2 + \psi_3) - \frac{H(0) m_0}{\sum_{X_i = 0} w_i}\psi_2 \\
  & = \beta_1 + \left[\frac{H(1)}{\bar{w}_{1\cdot}} - \frac{H(0)}{\bar{w}_{0\cdot}}\right]\psi_2 + \frac{H(1)}{\bar{w}_{1\cdot}}\psi_3
\end{align*}

Note that the bias of $\hat{\beta}_1$ depends on a few quantities. 
First, it depends on $\psi_2$ , which is the log-odds ratio of $X_i$ being missing for $T_i$ relative to $T_j = T_i - 1$. 
The second quantity is $H(1)$ which is approximately the probability that $X_i$ is missing when $X_i = 1$. 

When all studies have approximately the same estimation error variance so that $w_i \approx w$, then these biases approximately reduce to:
\[
\text{Bias}[\hat{\beta}_0] = \frac{P[R = 0 | X = 0]}{w} \psi_2
\qquad
\text{Bias}[\hat{\beta}_0] = \frac{P[R = 0 | X = 1] - P[R = 0 | X = 0]}{w} \psi_2 + \frac{P[R = 0 | X = 1]}{w} \psi_3
\]
Note that $P[R = 0 | X = 1] - P[R = 0 | X = 0]$ will be greater if $R$ and $X$ are strongly correlated.
Further $\psi_2$ will be larger if missingness is more strongly related to $T$, and $\psi_3$ will be larger if missingness has a stronger relationship to $T$ when $X = 1$. 
Finally, biases will be larger when $P[R = 0 | X]$ is larger, which means that the bias increases as a function of the proportion of missingness.

An alternative way to see this is to write $\pi_1 = P[R = 0 | X = 1] = a P[R = 0 | X = 0] = a \pi_0$. Then we can write:
\[
\text{Bias}(\hat{\beta}_0) = \frac{\pi_0 \psi_2}{w}
\]
which shows that bias in $\hat{\beta}_0$ increases as more effects for which $X_i = 0$ are missing and if missingness is strongly related to $\psi_2$.

Likewise:
\[
\text{Bias}(\hat{\beta}_0) = \frac{\pi_1}{w}(\psi_2 + \psi_3) - \frac{\pi_0}{w}\psi_2
\]
Here, we see that bias depends on the fraction of incomplete cases and the strength of the relationship between missingness and $X$ and $T$.

- Plots? [NOTE: How do relate the $\psi$ parameters to the $\beta$ parameters?]
One idea is to relate $P[R | X]$ to correlation between $X$ and $R$, since they're both binary, Kendall's tau and the pearson correlation are the same.
We could also say that $\psi_2$ is a log-odds ratio. 
We could posit low, medium, and large log-odds ratios.
$\gamma_3$ is a difference in log-odds ratios. 
Maybe we write:
\[
\frac{P[R = 0 | X = 1]}{w} (\psi_2 + \psi_3) - \frac{P[R = 0 | X = 0]}{w} \psi_2
\]
where $\psi_2 + \psi_3$ is the log OR for $X = 1$ and $\psi_2$ is the log OR for $X = 0$. 
Plot 1: $P[R= 0 | X = 0]$ is on the $x$-axis. Bias is on the $y$-axis. Lines are colored by values of $\psi_2$ that are small, medium, and large according to some convention.

Plot 2: $P[R = 0 | X = 1]$ is on the $x$-axis, bias is the $y$-axis, lines are colored according to $P[R = 0 | X = 0]$. Facet by $\psi_2 + \psi_3$ (low, med, hi) and $\psi_3$ (0, lo, med)


Different idea: We know from Chen, Cohen, \& Chen, that conventions for small, medium, and large odds ratios are about 1.4, 2.4, 4.1. This gives some idea of values for $\psi_2$ except that odds ratios assume a one unit change in the predictor (in this case $T$). A one unit change in $T$ when it is a SMD is huge. So maybe we scale these values to account for the difference between large and small effects $T_{big} - T_{small} = 0.6$, or small effects vs. null effects of about 0.2. In either case, we're setting $\Delta_T$ to be a meaningful difference in $T$ values and then backing out $\psi_2^* = \Delta_T \psi_2$ where $\psi_2$ is one of the conventions from Chen et al.

We can do a similar thing for $\beta_1$ where we note that $\psi_3$ is a difference in odds ratios. Based on the conventions for odds ratios in Chen et al, we can say that differences in odds ratios that are meaningful might range from 0 to 2. But again, we scale those by $\Delta_T$. 
...
\[
E[T | X, v, R \in \mathcal{R}_j] = \frac{\sum_{r \in \mathcal{R}_j}p(R = r | X, v) \left[X^T \beta + (1 - G_j(X, v)) \sum_{k = 0}^{n_j} \psi_{kj} f_{kj}'(X^T \beta, X, v)\right]}{\sum_{r \in \mathcal{R}_j} p(R = r | X, v)}
\]

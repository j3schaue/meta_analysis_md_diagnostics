...
\textcolor{blue}{[Comment-J] May start to use a term "shifting-case"?}
...
\textcolor{blue}{[Comment-J] What about the terminology "mixed-effect" meta-regression model? Although not sure whether this makes a difference. 
Isn't the subgroup analysis usually for binary/categorical variable?}
...
\]
\textcolor{blue}{[Comment-J] Maybe "$=p(\mathbf{R|\psi})$" to be consistent?}
MCAR implies that probability that a given value is missing is unrelated to anything observed or unobserved, but only with the missingness parameter ($\psi$).
...
\colortext{blue}{[Comment-J] When it says "the sources," does it mean missingness mechanism?}
...
\label{eq:pr_xv}
\end{equation}
\textcolor{blue}{[Comment-J]Can I say that this denominator is to presesnt the marginal distribution of R (across $T_i$)?}
...
\textcolor{blue}{[Comment-J] I'd like to clarify my understanding with an example scenario in page 5. $R_1=[1,1], R_2 = [1,0]$ which are included in the analysis. Then, $\mathcal{R}_j = \{[1,1], [1,0]\}$. Then, $\mathcal{R}=\{[1,1],[1,0],[0,1],[0,0]\}$ which include the cases with unobserved $X_1$. Thus, as the probability of the excluded pattererns ($\{[0,1], [0,0]\}$) gets greater, the bias is also greater. Am I understanding the notations correctly?}
...
\label{eq:cc_mnar}
\end{equation}
\textcolor{blue}{[Comment-J] Please reject this revision if you intentionally excluded $\eta$ and $\psi$ from the equation.}
...
\end{equation}
\textcolor{blue}{\textit{[Comment-J] (1) Are we assuming $\psi_0=0$ here? (2) Just one suggestion: How about changing the model to $logit[p(R_i=0)]$ to have a consistent direction with other components to bias ($H(0)$, $\tau^2 + v$)? And when we say "missingness model", then usually the probability of being missing is modeled. So, this made me pause to think about the direction, especially when Figure 1 is explained later and in the caption of Figure 1.}}
...
The bias of $\hat{\beta}_{1C}$ also increases as a function of $p(R = 0 | X = 1) - p(R = 0 | X = 0)$, which can be thought of as a difference in observation rates \textcolor{blue}{([Comment-J] You mean "missingness rates" ($R=0$), right?)} between $X_i = 1$ and $X_i = 0$.
...
As noted above, the missingness bias and omitted variable bias need not be in the same direction. 
...
Further, issues of multicollinearity and confounding in meta-regression, including those discussed by @lipseyThoseConfoundedModerators2003, would suggest that omitting variables in a shifting-case analysis are likely to induce bias.

Based on these results, our primary recommendation is that analysts attempt to understand the missingness mechanisms in their data.
...
Research on and clear implementation of these methods for meta-regression would seem to be of great use for meta-analysts.

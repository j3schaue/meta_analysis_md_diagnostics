...
p(T_i | X_i, v_i, R_i = \mathbbm{1}, \eta) 
  ...
  & = \frac{G(\psi) p(T_i | x_i, v_i, \eta)}{G(\psi)} \\
  & = p(T_i | x_i, v_i, \eta)
\end{align*}
...
Further, denote $\mathbf{X}_R$ as the matrix of covariates such $R_i = \mathbbm{1}$. 
If we denote $\mathbf{W}_R = diag(v_i + \tau^2): R_i = \mathbbm{1}$, the complete-case analysis will estimate coefficients as:
\begin{align*}
\hat{\beta} 
  & = (\mathbf{X}_R^T W_R \mathbf{X}_R)^{-1} \mathbf{X}_R^T \mathbf{W}_R E[\mathbf{T}_R | \mathbf{X}_R, \mathbf{v}_R, R = \mathbbm{1}] \\
  & = (\mathbf{X}_R^T \mathbf{W}_R \mathbf{X}_R)^{-1} \mathbf{X}_R^T \mathbf{W}_R \left[\mathbf{X}^T\beta + \mathbf{W}_R^{-1}\mathbf{H}_R  \mathbf{f}_R \psi\right] \\
\end{align*}
where $\mathbf{H}_R = diag(H_{ij})$ and $\mathbf{f}_R = [f_{i1}', \ldots, f_{in}']$.
Thus, the bias of the regression coefficients is given by 
\[
(\mathbf{X}_R^T \mathbf{W}_R \mathbf{X}_R)^{-1} \mathbf{X}_R^T \mathbf{H}_R  \mathbf{f}_R \psi
\]

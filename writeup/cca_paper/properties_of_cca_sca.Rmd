...
Finally, $\psi_1$ is an odds ratio, which reflects the odds of a complete case for $T_i$ versus $T_i - 1$.
...
However, the actual size of $\psi_3$ will depend on the scale of $T_i$.
...
Thus, we consider $\Delta_T = 0.1, 0.2, 0.3$ to be of interest on this scale.
These conventions for odds ratios and $\Delta_T$ would imply that $\psi_1$ might range from 0.15 to 1.35. 
...
However if $\tau^2 + v_i$ is larger and the probability of a complete case is strongly related to $T_i$ (i.e., $\psi_1$ is large), then the bias can be greater than 0.1 or even 0.15 (Cohen's $d$).

\begin{figure}
\begin{center}
\includegraphics[width = \textwidth]{./graphics/delta_plot}
\end{center}

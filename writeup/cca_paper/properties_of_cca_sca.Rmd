...
\textcolor{blue}{[JL - How much detail do you want to have in here? I think we can just say we assume independent effect sizes in this study and not provide the detailed model formulation with dependent effect sizes. If necessary, we may include the formulation in Appendix instead. Dependent effect size case will just have a different weight matrix, so I think this seems to be enough.
Add a short discussion as a footnote can be another option I think. If this gets longer in the main text, it may prevent from following the idea.]}
...
\textcolor{blue}{[JL - I think the notations in this paragraph are not clear to me with introducing $X_{US}$. Can we simplify this?]}
...
\textcolor{blue}{[JL - This may be supported by Little \& Rubin (2002, CH3), citing "Glynn \& Laird (1986)". Note that this is in case of regression coefficients.]}
...
\textcolor{blue}{[JL - Does it indicate $R_i \perp v_i | T_i, X_i$? Is there a reason not including $v_i$ in the logit model?
Also, what about start from MAR scenario example and just mention that under MNAR assumption, the bias will be the same in conclusion? Because MAR mechanism is more commonly assumed in practice.]}
...
\textcolor{blue}{[JL - I could follow this paragraph. However, the conclusion that assuming MAR and MNAR will give us the same amount of bias is not a bit intuitive. This makes sense because our bias approximation involves the derivative with respect to only $T_i$, but others may question on this..? Also, does it indicate that a form of selection model doesn't change bias, and only $\psi_1$ with $T_i$ matters?]}
...
\textcolor{blue}{[JL - How about adding a few sentences about linking intercept and slope together? I remember you briefly talked about this concept last week. For example, if intercept is biased (related to $H_1(0)$) but slope is unbiased (related to $H_1(1) - H_1(0)$), then we would still have biased effect size estimates for each group (X = 0 vs. X = 1). This may help readers understand about this example (binary covariate case) better.]}

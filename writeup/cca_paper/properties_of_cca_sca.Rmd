...
Denote the matrix $\mathbf{R} = [R_1, \ldots, R_k]$ of all missingness indicators.
...
As argued below, complete- and available-case meta-regressions typically rely on a subset of the $\mathbf{X}$ matrix that omits rows and/or columns.
...
Shifting-case analyses omit covariates *and* missing data, so that $\mathbf{X}_S \equiv \mathbf{X}_A$ contains a subset of the columns of $\mathcal{X}$ and only rows from that subset that are not missing any variables.
...
A general result based on the approximation in the previous section follows from the fact that a complete-case analysis involves $R_i = \mathbbm{1}$.
Let $C$ index all relevant effects $i$ such that $R_i = \mathbbm{1}$, so that  $\mathbf{X}_C$ is the matrix of covariates such $R_i = \mathbbm{1}$, $\mathbf{T}_C$ is the corresponding subset of effect estimates, and $\mathbf{W}_C = diag(v_i + \tau^2): R_i = \mathbbm{1}$ is the corresponding subset of weights. 
The complete-case analysis estimates coefficients $\beta$ with:
\[
(\mathbf{X}_C^T W_R \mathbf{X}_C)^{-1} \mathbf{X}_C^T \mathbf{W}_C \mathbf{T}_C
\]

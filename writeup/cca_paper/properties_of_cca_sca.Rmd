...
Finally, $R_i \perp T_i$ if covariates are MNAR such that $R_i \perp T_i | (X_i, v_i)$, so that missingness depends solely on $T_i$.
...
However, it is possible to have MAR and MNAR mechanisms for where $R_i$ depends on $T_i$.
...
Because \@ref(eq:cc-loglinear) depends on $T_i$, $\delta_{ij} \neq 0$ for this selection model regardless of MAR or MNAR (i.e., regardless of whether $\psi_2 = 0$), the CCA estimators may be biased.
...
\textcolor{orange}{
NOTE2: We must re-write the empirical example stuff, since I don't think it's well-justified. Maybe we do some imputation scheme? We could also set this apart in a separate section.
My point here is this: quantities like $P[R = 1 | X]$ is not estimable from the data. 
We could do something like simplifying our selection model so that it's MAR, but that doesn't fix this issue (though we could estimate $\psi_1$).
One approach would be to use PMM to impute missing Xs. Then we could compute a pooled estimate of $P[R = 1 | X]$, which drives the bias of the intercept and slope.
}

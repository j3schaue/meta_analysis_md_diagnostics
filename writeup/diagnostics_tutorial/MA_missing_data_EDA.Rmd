...
First, data could be missing on individual participants within studies, including their outcomes in the study or other characteristics (e.g., their age, race, prior substance use) [e.g., @higginsImputationMethodsMissing2008]. Although this type of missingness cannot be controlled by a meta-analyst, it may impact the precision of each primayr study's effect size estimates.
...
The focus of the tutorial involves a dataset on substance abuse interventions typical of those used in meta-regression model. \textcolor{blue}{([JL] Deleted redundant phase as in the previous paragraph.)}
The following section describes a meta-analytic dataset on substance abuse interventions that will be used to demonstrate key concepts in *exploratory missingness analyses* (EMA). \textcolor{blue}{([JL] Maybe add one sentence about this defined terminology we'll use throughout the paper?)}
...
In all, their raw data totaled some $k = 328$ effect estimates and $p = 43$ variables for each effect. \textcolor{blue}{([JL] Does the 43 "variables" mean covariates in meta-regression?)}
...
@schaferMultipleImputationPrimer199 suggests that if less than 5% of the data are missing, this can be inconsequential, and @bennettHowCanDeal2001 argues that a missingness rate of over 10% can induce non-negligible bias in statistical analyses.
...
Missingness rates of additional variables are displayed in Table \@ref(tab:pcts). \textcolor{blue}{([JL] I think the information in Table 1 is not well align with these three quantities here..?)}
...
\textcolor{blue}{([JL] We haven't reached to visual inspection yet, so how about not mentioning Figure 2 yet? Or, I think we may need a brief explanation of Figure 2.)}
...
\textcolor{blue}{[JL] Maybe title as "Variable missing plots" aligning with the previous section?}
...
```{r misscor, echo=FALSE, warning = FALSE, fig.width=6.5, fig.height=5, fig.pos = 'p', fig.cap="\\label{fig:misscor} Missingness patterns plot. *This plot shows the pairwise correlation of missingnes for each variable in the data. Each tile refers to a pair of variables and is shaded according to the correlation.*"}
library(viridis)
